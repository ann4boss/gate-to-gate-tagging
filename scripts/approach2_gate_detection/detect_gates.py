...
def compare_to_gt(predicted: dict, gt_path: str) -> None:
    ...
    print("-" * 52)
    for i, (gt_f, err) in enumerate(zip(gt_frames, errors)):i
    ...
    print(f"  Within  3 fr   : {w3}/{n}  ({w3/n*100:.0f}%)")
    print(f"  Within  8 fr   : {w8}/{n}  ({w8/n*100:.0f}%)")
    print(f"  Within 15 fr   : {w15}/{n}  ({w15/n*100:.0f}%)")
    print(f"  Within 30 fr   : {w30}/{n}  ({w30/n*100:.0f}%)")
